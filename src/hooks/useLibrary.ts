...
import { books as initialBooks, students as initialStudents, createRecords } from '../contants/data';
...
export const useLibrary = () => {
    ...
    const analytics = useMemo(() => {
        ...
        active.forEach(r => {
            ...
            if (book) categories.set(book.category, (categories.get(book.category) || 0) + 1);
        });
        ...
        return {
            ...
            topBooks: [...books].sort((a, b) => b.borrowCount - a.borrowCount).slice(0, 5),
            categories,
        };
    }, [books, records, bookMap, overdueRecords]);

    const issueBook = useCallback((bookId: string, studentId: string) => {
        const book = bookMap.get(bookId);
        const student = studentMap.get(studentId);

        if (!book || !student) return alert('Invalid selection');
        if (book.availableCopies <= 0) return alert('No copies available');
        if (student.borrowedBooks.includes(bookId)) return alert('Already borrowed');
        if (student.borrowedBooks.length >= 3) return alert(' Maximum 3 books limit');
        ...
        setBooks(prev => prev.map(b => b.id === bookId
            ? { ...b, availableCopies: b.availableCopies - 1, borrowCount: b.borrowCount + 1 }
            : b
        ));
    ...
    const returnBook = useCallback((recordId: string) => {
        ...
        if (today > record.dueDate) {
            const days = Math.floor((today.getTime() - record.dueDate.getTime()) / (1000 * 60 * 60 * 24));
        ...
        setBooks(prev => prev.map(b => b.id === record.bookId
            ? { ...b, availableCopies: b.availableCopies + 1 }
            : b
        ));
        setStudents(prev => prev.map(s => s.id === record.studentId
            ? { ...s, borrowedBooks: s.borrowedBooks.filter(id => id !== record.bookId) }
            : s
        ));
    ...
    return { books, students, records, bookMap, studentMap, overdueRecords, analytics, issueBook, returnBook };
};
